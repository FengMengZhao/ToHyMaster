# 数据库设计(2017-Q3 2018-Q3-4更新)

---

**问题：**

1). 随着业务的增长，造成单个数据库内的表的数量很多，同一个数据库会遇到性能瓶颈，这时候需要垂直分库。基本的垂直分库的方式是按照业务将不同业务表分放在不同的数据库中。这是会出现一个问题：如果应用的功能涉及到不同的业务，需要怎么办？

> 回答：<br>

就拿案件审批的例子来说，民事、刑事和行政的案件按照垂直分库的原则数据落在不同的库中，但是对于审批流程来说，这些案件都需要，需要如何处理这些问题？

如果是从应用层的角度来解决这个问题：需要为应用提供不同的模块组件，不同模块基于不同的库对外提供接口服务，审批系统需要调用各个子模块的接口获取数据并进行拼接(如果涉及到性能的问题，可以使用缓存)。

2). 为什么认为主键应该是单列的，避免使用联合主键？

> 回答：<br>

1. 首先我理解主键的作用是能够唯一标识出来一条记录，最好是程序自动生成的逻辑主键，而不是具有业务含义的业务主键。如果是业务主键，就存在着业务变化导致主键变化的情况，而一个表的主键往往不是独立的，很可能是其他表的外键，如果更改会增加整体的维护成本。
2. 其次，如果一个表中的一个业务字段不能够标识一条记录，这时候就可能采取联合主键的方法，这样会给表之间的关联带来复杂度并且如果业务字段发生变化就非常难维护。正确的方法就是定义一个逻辑主键，和其他表的关联通过这个逻辑主键来完成。

3). 为什么说水平分表后，一个表中数据过多分为了几个表存储，这些表更容易维护？

> 回答：<br>

个人理解：水平分表的目的是让一个表的数据可以分散到多个表中，让数据分散到哪些表中和按照什么规则进行分散是我们定的，也是可以更改的。有了这样的灵活性，我们可以分析数据的热度，调整分表策略，引导热度数据到指定表中。

---

**近期工作：**

负责司法大数据-数据质检系统上云(阿里云)。

数据库设计相关工作：

- 原始库到标准库标准的制定
- 质检结果展示系统数据库设计
- 阿里云DataWorks数据质检过程数据库设计
...
在现实项目中，往往是这两种情况兼而有之，这就需要做出权衡，甚至既需要垂直切分，又需要水平切分。举例，首先对数据库进行垂直切分，然后，再针对一部分表，通常是用户数据表，进行水平切分。
...
2. 第二范式: 数据库表中不存在非关键字段对任一候选键的部分函数依赖，也即所有非关键字段都完全依赖于任意一组候选关键字.
...
> 第一范式是对属性原子性的约束，要求属性具有原子性，不可再分割；<br>
第二范式是对记录唯一性的约束，要求记录有唯一标识，即实体的唯一性；<br>
第三范式是对冗余字段的约束，即任何字段不能由其他字段派生而来；<br>
